...
export default function ShortCard({ post }) {
  ...
  return (
    <div className="bg-black rounded-lg shadow p-4 mb-4 h-screen flex flex-col justify-center">
      <div className="flex items-center space-x-2 mb-2">
        ...
        />
        <div>
          <p className="font-semibold text-white">{post.users.name}</p>
          <p className="text-gray-300 text-sm">@{post.users.username}</p>
        </div>
      </div>
      <video
        src={post.media_url}
      ...
      <div className="flex items-center space-x-4 mt-2">
        <button onClick={handleLike} className="flex items-center text-white hover:text-red-500">
